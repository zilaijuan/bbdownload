...
public class IndexController {
    @Autowired
    private CrawlServer buCrawlServer;
    @Autowired
    ...
    public String index(Model model){
        // 加缓存，这样就不用每次都请求了。
        List<SourceName> sourceNames = buCrawlServer.getSourceName();
    ...
    public String detail(Model model,String id,String source){
        // 加缓存，这样就不用每次都请求了。
        List<SourceDetail> sourceDetails = buCrawlServer.getSourceDetail(id);
